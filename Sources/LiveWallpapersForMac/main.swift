...
enum AppBrand {
    ...
    static let legacyLaunchAgentLabel = "local.codex.\(legacySupportDirectoryName).login"
    static let legacyCodexLaunchAgentLabel = "local.codex.LiveWallpapersForMac.login"
...
enum LaunchAtLoginManager {
    ...
    static var isEnabled: Bool {
        FileManager.default.fileExists(atPath: launchAgentURL(label: label).path)
            || FileManager.default.fileExists(atPath: launchAgentURL(label: AppBrand.legacyLaunchAgentLabel).path)
            || FileManager.default.fileExists(atPath: launchAgentURL(label: AppBrand.legacyCodexLaunchAgentLabel).path)
    }
    ...
    private static func installLaunchAgent() throws {
        try removeLaunchAgent(label: AppBrand.legacyLaunchAgentLabel)
        try removeLaunchAgent(label: AppBrand.legacyCodexLaunchAgentLabel)
